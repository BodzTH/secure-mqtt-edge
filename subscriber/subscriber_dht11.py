...
def decrypt_and_verify(raw: bytes):
    ...
    plaintext = (unpadder.update(padded_data) + unpadder.finalize()).decode("utf-8")
...
class DHT11Dashboard(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # ── Log box ──────────────────────────────────────────────────────────
        # Fixed: Tuple padding removed from tk.Frame constructor
    ...
    def _start_mqtt(self):
        # Initialized Paho MQTT with VERSION2
        self._client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION2, client_id="dht11_subscriber")
